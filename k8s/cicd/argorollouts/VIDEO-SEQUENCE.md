...
**확인 사항**:
- ScaledDown된 ReplicaSet이 있으면 정리됨 (자동)
- 현재 Active 버전 확인
...
**순서**:
1. 브라우저 열기
2. 주소창에 입력: `http://localhost:9003/rollouts/`
3. Enter
4. 대시보드 로딩 대기

**화면 확인**:
- Argo Rollouts 대시보드 표시
- `frontend` Rollout 보임

---

### Step 2: 크리스마스 버전 배포 (Blue-Green 시작)
...
- **Step 1**: 5초 (대시보드 접속)
- **Step 2**: 15초 (배포 + 대기)
- **Step 3**: 10초 (Preview 확인)
- **Step 4**: 10초 (Promote + 확인)
- **Step 5**: 15초 (롤백 배포 + 대기)
- **Step 6**: 10초 (Preview 확인)
- **Step 7**: 10초 (Promote + 확인)

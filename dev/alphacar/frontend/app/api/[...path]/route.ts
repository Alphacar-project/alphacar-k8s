...
const getBackendUrl = (path: string): string | null => {
  ...
  if (
    ...
    return `http://main-backend.apc-be-ns.svc.cluster.local:3002/${path}`;
  }
  ...
  if (
    ...
    if (
      ...
      return `http://main-backend.apc-be-ns.svc.cluster.local:3002/${path.replace('vehicles/', '')}`;
    }
  ...
  if (path.startsWith('mypage') || path.startsWith('auth')) {
    return `http://mypage-backend.apc-be-ns.svc.cluster.local:3006/${path}`;
  }

  // Search Backend (포트 3007)
  if (path.startsWith('search')) {
    return `http://search-backend.apc-be-ns.svc.cluster.local:3007/${path}`;
  }

  // News Backend (포트 3008)
  // 백엔드 컨트롤러는 @Controller('drive')를 사용하므로 /drive로 변환
  if (path.startsWith('news')) {
    const drivePath = path.replace('news', 'drive');
    return `http://news-backend.apc-be-ns.svc.cluster.local:3008/${drivePath}`;
  }

  // AI Chat Backend (포트 4000)
  if (path.startsWith('chat')) {
    return `http://aichat-backend.apc-be-ns.svc.cluster.local:4000/${path}`;
  }

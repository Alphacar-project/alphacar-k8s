...
import { useEffect, useState, useCallback, CSSProperties, Suspense } from "react";
...
function handleOAuthCallback(code: string, state: string | null) {
  // Google Cloud Console에 등록된 URI와 일치 (슬래시 없음)
...
function HomePageContent() {
    ...
    if (code && state) {
      handleOAuthCallback(code, state)
        .then((response) => {
          ...
          localStorage.setItem("alphacarToken", access_token);
  ...
  const fetchMyFavorites = useCallback(async (uid: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/favorites/list?userId=${uid}`); // VirtualService /api/favorites 규칙 사용
      if (res.ok) {
        ...
        <section style={{ margin: "50px auto 40px", padding: "0 40px", textAlign: "center" }}>
          ...
          <form onSubmit={handleSearchSubmit} style={{ display: "inline-flex", alignItems: "center", gap: "12px" }}>
            <div style={{ position: "relative", width: "720px", maxWidth: "90vw" }}>
              <input type="text" placeholder="어떤 차를 찾으세요?giri " value={searchText} onChange={(e) => setSearchText(e.target.value)} style={{ width: "100%", height: "56px", padding: "0 22px", borderRadius: "999px", border: "1px solid #e5e7eb", fontSize: "17px", outline: "none" }} />
            </div>

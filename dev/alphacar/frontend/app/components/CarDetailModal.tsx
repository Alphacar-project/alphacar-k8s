...
export default function CarDetailModal({ car, onClose }: CarDetailModalProps) {
  ...
  useEffect(() => {
    ...
    if (targetId) {
      setLoading(true);
      const apiUrl = `/api/vehicles/detail?trimId=${encodeURIComponent(targetId)}`; // Next.js API Route 사용
    ...
    if (storedUserId && targetId) {
      ...
      // 찜 상태 확인
      fetch(`/api/favorites/status?userId=${storedUserId}&vehicleId=${targetId}`) // VirtualService /api/favorites 규칙 사용
        .then(res => res.json())
